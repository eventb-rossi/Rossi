...
/// Component key = entry path minus the proof extension (`a/M1.bpr` →
/// `a/M1`). The archive path stays in the key — that is how eventb-checker
...
mod tests {
    ...
    fn bpo_is_the_obligation_source_when_present() {
        ...
        assert_eq!(report.diagnostics[0].origin, "p/M0");
        assert_eq!(
    ...
    #[test]
    fn component_join_is_per_entry_path() {
