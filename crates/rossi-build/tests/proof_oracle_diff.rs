...
//! exceed that. Such an oracle drops the
...
//!
//! The corpus directory is taken from `EVENTB_CORPUS_DIR`, falling back to
//! the sibling `eventb-models-collection` checkout (the test skips when
...
fn proof_oracle_diff() {
    ...
    let Some(dir) = corpus_dir() else {
        eprintln!("SKIP proof_oracle_diff: no corpus (set EVENTB_CORPUS_DIR)");
        return;
    };
    ...
    for e in &errors {
        if field(e, "ruleId") == "EB017" {
            ...
            if message.contains("JAXP") || message.contains("jdk.xml") {
                // `M0.bpr` (possibly path-qualified) → `M0`.
